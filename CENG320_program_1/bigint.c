...
void bigint_trim_in_place(bigint b);
...
int bigint_to_int(bigint b) {
  ...
  bigint tmp1;
  bigint_trim_in_place(b); /* make a trimmed copy */
...
bigint bigint_adc(bigint l, bigint r, chunk carry) {
  ...
  bigchunk tmpsum;
  chunk lsign,rsign;
  // make sure that l is the one with fewer chunks
  if(l->size > r->size)
    { // if l has more chunks than r, then swap them
      sum = l;  
      l = r;
      r = sum;
    }
  /* Allocate space for the result. Allocate one extra chunk to make
     sure overflow cannot occur */
  sum = bigint_alloc(r->size + 1);
  /* Perform the addition for all matching chunks. */
  for (i = 0; i < l->size; i++) {
    /* add the current block of bits */
    tmpsum = (bigchunk)l->blks[i] + (bigchunk)r->blks[i] + (bigchunk)carry;
    sum->blks[i] = tmpsum & CHUNKMASK;
    /* calculate the carry bit for the next chunk */
    carry = (tmpsum >> BITSPERCHUNK) & CHUNKMASK;
  }
  // Get a chunk that contains the sign of l extended across it.
  if((schunk)l->blks[l->size-1] < 0)
    lsign = (schunk)-1;
  else
    lsign = 0;
  // If r has more chunks than l, then add chunks containing the sign
  // bit of l to the remaining chunks in r.
  for (i = l->size; i < r->size; i++) {
    /* add the current block of bits */
    tmpsum = (bigchunk)lsign + (bigchunk)r->blks[i] + (bigchunk)carry;
    sum->blks[i] = tmpsum & CHUNKMASK;
    /* calculate the carry bit for the next chunk */
    carry = (tmpsum >> BITSPERCHUNK) & CHUNKMASK;
  }
  // Get a chunk that contains the sign of r extended across it.
  if((schunk)r->blks[r->size-1] < 0)
    rsign = (schunk)-1;
  else
    rsign = 0;
  // calculate top chunk of result
  sum->blks[r->size] = lsign + rsign + carry;
  bigint_trim_in_place(sum);
  return sum;
#else
...
int bigint_cmp(bigint l, bigint r) {
  ...
  int lneg,rneg,lsize,rsize;
  // get the number of chunks in l and r
  lsize = l->size;
  rsize = r->size;
  // set flags to indicate the signs of l and r
  lneg = ((schunk)l->blks[lsize-1]) < 0 ? 1 : 0;
  rneg = ((schunk)r->blks[rsize-1]) < 0 ? 1 : 0;
  // if the signs of l and r differ, then it is easy
  if (lneg ^ rneg)
      return lneg ? -1 : 1;

  // if the signs are the same, but the lengths differ, then it is easy
  if(lsize != rsize)
     return lneg ? (lsize < rsize ? 1 : -1) : (lsize < rsize ? -1 : 1);

  // if the signs and lengths are the same, then we have to compare
  // chunk-by-chunk
  --lsize;
  if(lneg)
    { // the first chunk is signed
      if(((schunk)l->blks[lsize]) < ((schunk)r->blks[lsize]))
	return -1;
      if(((schunk)l->blks[lsize]) > ((schunk)r->blks[lsize]))
	return 1;
      // the remaining chunks are unsigned
      while(--lsize >= 0)
	{
	  if(l->blks[lsize] < r->blks[lsize])
	    return -1;
	  if(l->blks[lsize] > r->blks[lsize])
	    return 1;
	}
  ...
  // If every chunk matches, then they are eqal.
  return 0;      
